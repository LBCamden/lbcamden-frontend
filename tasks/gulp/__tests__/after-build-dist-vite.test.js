...
describe('dist/', () => {
  const version = require(path.join('../../../', configPaths.package_vite, 'package.json')).version
  ...
  describe('assets/', () => {
    it('should include the same files as in src/assets', () => {
      ...
      const actualDistAssets = () => {
        return recursive(path.join(configPaths.dist_vite, 'assets')).then(
          files => {
            return files
              // Remove /package prefix from filenames
              .map(file => file.replace(/^package-vite\/dist\/assets\//, ''))
              // Sort to make comparison easier
              .sort()
          },
          error => {
  ...
  describe(`lbcamden-frontend-${version}.min.css`, () => {
    const stylesheet = lib.readFileContents(path.join(configPaths.dist_vite, `lbcamden-frontend-${version}.min.css`))
  ...
  describe(`lbcamden-frontend-${version}.min.js`, () => {
    const javascript = lib.readFileContents(path.join(configPaths.dist_vite, `lbcamden-frontend-${version}.min.js`))
  ...
  describe(`govuk-frontend-${govukVersion}.min.js`, () => {
    const javascript = lib.readFileContents(path.join(configPaths.dist_vite, `govuk-frontend-${govukVersion}.min.js`))
