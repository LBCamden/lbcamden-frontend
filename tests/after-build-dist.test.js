...
describe("dist/", () => {
  const version = require(
    path.join("../", configPaths.package, "package.json"),
  ).version;
  ...
  describe("assets/", () => {
    it("should include the same files as in src/assets", () => {
      ...
      // to be present
      Promise.all([actualDistAssets(), expectedDistAssets()]).then(
        (results) => {
  ...
  describe(`lbcamden-frontend-${version}.min.css`, () => {
    const stylesheet = lib.readFileContents(
      path.join(configPaths.dist, `lbcamden-frontend-${version}.min.css`),
    );
  ...
  describe(`lbcamden-frontend-${version}.min.js`, () => {
    const javascript = lib.readFileContents(
      path.join(configPaths.dist, `lbcamden-frontend-${version}.min.js`),
    );
